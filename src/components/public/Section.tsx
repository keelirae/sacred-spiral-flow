...
export default function Section({
  ...
  return <section id={id} className={`relative ${bg} ${anchorOffset}`}>
      ...
      <div className="relative mx-auto max-w-3xl md:max-w-4xl lg:max-w-[960px] px-4 py-8 md:py-10 lg:py-14 text-center">
        {(eyebrow || title) && <header className="relative mb-6">
            ...
            {title && <h2 className="mt-2 text-3xl font-serif tracking-tight text-foreground md:text-4xl">{title}</h2>}
          </header>}
        {children}
      </div>
    </section>;
